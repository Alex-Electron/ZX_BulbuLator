...
![Финал демки Rage на машине Пентагон, считанный из буфера кадра платы](docs/images/demo-rage-pentagon.png)

*Финал демки Rage на машине Пентагон. Восемь цветных секторов идут и по бумаге, и по бордюру, поэтому этим экраном проверяется шаг бордюра Пентагона в 2 пикселя. Кадр считан из буфера кадра платы, а не сфотографирован с монитора.*

![ZX BulbaNavigator, файловый менеджер на плате](research/15-pentagon/images/nav-browser.png)

*ZX BulbaNavigator, оболочка на стороне ARM: файловый менеджер в стиле DOS Navigator, который загружает ленты, снэпшоты, образы дисков и музыку с SD-карты. Больше скриншотов — в [Шаге 15](research/15-pentagon/).*

Самое большое отличие от оригинальных ядер — это память. MiST управляет внешним контроллером SDRAM; здесь собственные 128 КБ Спектрума лежат во встроенной BRAM и подключены к ядру напрямую, а расширенные банки Пентагона 8..63 живут в DDR3 процессорной системы и читаются через AXI — это снимает с платы много боли по таймингам и разводке.

Этот репозиторий — рабочий блокнот и сборник идей. Он пополняется по мере проверки всего на реальном оборудовании.

## Как попробовать

Чтобы попробовать, собирать ничего не нужно: готовые файлы лежат в
[`research/15-pentagon/bitstreams/`](research/15-pentagon/bitstreams/). Как собрать всё самому, подробно написано в [`research/15-pentagon/BUILDING.ru.md`](research/15-pentagon/BUILDING.ru.md).

**Железо.**

- Плата EBAZ4205 с Zynq-7010, переведённая на загрузку с карты microSD. Для этого нужен один резистор — R2577.
  [Шаг 0](research/00-setup/) показывает, где он находится и как подать на плату питание (5 В или 12 В).
- HDMI и клавиатура PS/2. На плате нет ни одного из этих разъёмов, поэтому оба заведены на выводы ПЛИС; у меня они
  стоят на самодельном шилде. Распиновка приведена в
  [`bulbulator_ddr.xdc`](research/15-pentagon/sources/bulbulator_ddr.xdc): HDMI TMDS на F19/F20 (такт),
  D19/D20, C20/B20 и B19/A20, такт PS/2 на G19, данные на H20. Звук выводится через HDMI.
- Карта microSD, отформатированная в FAT32.

**Карта.** Обязателен только `BOOT.BIN`: без остального плата поднимается с ПЗУ, вшитым в
битстрим. Остальные файлы добавляют машины и устройства. Пути слева указаны относительно
`research/15-pentagon/`.

| Из репозитория | На карту | Что это |
|---|---|---|
| `bitstreams/BOOT_B0200_v0.15.448.BIN` | `BOOT.BIN` | загрузочный образ: начальный загрузчик, ядро ZX и прошивка оболочки |
| `bitstreams/ATLAS_B0200.bit.bin` | `CORES/ATLAS.BIT.BIN` | ядро ZX; оболочка перезагружает его отсюда при смене машины |
| `bitstreams/NES_CE29.bit.bin` | `CORES/NES.BIT.BIN` | NES / Денди |
| `roms/*.ROM` | `ROMS/` | наборы ПЗУ, выбираются для каждой машины опцией `ROM SET` |
| `roms/GS105B.ROM` | `GS/GS105B.ROM` | прошивка карты General Sound |
| `roms/ESXMMC.ROM` | `ROMS/ESXMMC.ROM` | ПЗУ DivMMC; самому esxDOS нужны ещё его каталоги `SYS` и `BIN` с [esxdos.org](http://www.esxdos.org/) на карте DivMMC (каталог `DIVMMC/` или образ `DIVMMC.IMG`) |

Игры, демки, образы дисков и музыка кладутся куда угодно. Полная раскладка карты — в
[`docs/SDCARD.ru.md`](research/15-pentagon/docs/SDCARD.ru.md).

**Первый запуск.** Вставь карту и включи питание платы. Поднимется ZX Spectrum 128 с открытым поверх навигатором.
Выбери `.tap`, `.tzx`, `.z80`, `.sna`, `.trd`, `.scl` или музыкальный файл и нажми Enter.

| Клавиша | Что делает |
|---|---|
| `F12` | спрятать или показать навигатор |
| `F9` | строка меню: машина, ПЗУ, диски, экран, лента, звук |
| `Enter` | открыть папку, загрузить программу, включить музыку |
| `F5` / `F6` / `F7` / `F8` | скопировать / переименовать или переместить / создать папку / удалить |
| `F10` | пауза машины |
| `F11` | жёсткий сброс, ОЗУ очищается |
| `Ctrl+Alt+Del` | мягкий сброс, ОЗУ сохраняется |
| `Ctrl+Alt+Ins` | NMI |
| `Num +` / `Num -` | громкость |
| `Esc` | назад или закрыть |

Настройки вступают в силу сразу, но на карту они попадают только после того, как ты выберешь в меню пункт **Save config**.
...
Самое интересное в этой плате — расположение памяти. Первые 128 КБ Спектрума лежат во встроенной Block RAM, которую текущая сборка почти заполняет (58.5 из 60 блоков), а 256 МБ DDR3 несут расширенные банки Пентагона и тройной буфер кадра, общий у процессора и видеотракта. Платы 7020 дают примерно втрое больше LUT и триггеров (53 200 LUT, 220 DSP), но блочной памяти лишь вдвое с небольшим (140 блоков) — а именно она у этого проекта кончается первой. Сборка по-прежнему ориентирована на 7010, которая есть у всех.

## Что работает, и что ещё в планах

### Работает на плате сегодня

**Машины.** ZX Spectrum 48K, 128K и Пентагон 128–1024K на одном ядре; NES/Денди — вторая платформа, своим ядром. Две синклеровские машины сверены
измерением, а не на глаз: их наборы тестов таймингов проходят целиком (у 48K — с выключенным NEMO-IDE, как на настоящем 48K: карта дешифрует порты неполно и отвечает как раз на те порты, которые читает тест). У Пентагона работают и
проверены страничность, наборы ПЗУ, дискета и тонкий бордюр в две точки, но **приёмка таймингов ещё
не прогонялась**.

**Звук.** Шесть источников, и все слышны: бипер, AY-3-8912 / YM2149F, Turbo Sound (второй AY),
SAA1099 на честных 8 МГц, SpecDrum и **General Sound — свой Z80 со своей памятью и четырёхканальным
ЦАП, играющий музыку**. Выход идёт аудиопакетами HDMI, с пик-метром по источникам: тишина сама по себе
ничего не доказывает. Внешний ЦАП пока в планах.

**Накопители.** TR-DOS / Beta Disk через WD1793 — чтение, **запись** и `FORMAT`, образы `.trd` и
`.scl`; NEMO-IDE читает `.hdf` байт в байт; DivMMC / esxDOS образом или папкой; Z-Controller. Образы
дискет листаются прямо из навигатора.

**Ввод.** Клавиатура PS/2 на двух контактах ПЛИС, джойстик Kempston, мышь Kempston (в том числе
управление с цифровой клавиатуры — для софта, который умеет только мышью) и раскладка джойстика на
каждую машину с визардом захвата кнопок.

**Лента.** `.tap`, `.tzx`, `.wav` и `.mp3`. Стандартная лента 48K грузится мгновенно: SMART LOAD её не проигрывает, а читает из файла Бейсик-загрузчик и блоки CODE, собирает образ памяти и запускает его как снэпшот. Всё остальное (турбо, свои загрузчики, программы 128K) идёт настоящими импульсами, в родном темпе или с разгоном 8×. На 128K и Пентагоне автостарт входит в загрузку через меню 128, `USR 0` или 48 BASIC с запертой страничностью — как нужно программе.

**Видео.** HDMI 720p50 с целочисленным увеличением на каждую машину, кроп по сторонам и панорама, и смешение кадров в духе ЭЛТ для демок, которые переключают экран каждый кадр.

### В планах

- **Настоящий дисковод** — вывести сигналы WD1793 на GPIO через преобразователь уровней 3,3 → 5 В,
  чтобы к плате можно было подключить физический привод. Это то, что хочется сделать больше всего.
- **ATM Turbo 2 и CP/M** ([Шаг 16](https://github.com/Alex-Electron/ZX_BulbuLator/issues/109)) — новая машина с текстовым режимом
  80×25 и ОЗУ по адресу `#0000`, так что CP/M получает нормальный экран на 80 колонок, а Turbo Pascal
  запускается прямо на плате.
- **Сеть** ([Шаг 17](https://github.com/Alex-Electron/ZX_BulbuLator/issues/82)) — веб-пульт с живым экраном, клавиатурой и файловым
  менеджером карты. Сторона прошивки написана; Ethernet этой платы разведён на выводы ПЛИС, а не
  процессора, поэтому нужен битстрим, выводящий контроллер на ноги микросхемы PHY.
- **[ULAplus и палитра на 64 цвета](https://github.com/Alex-Electron/ZX_BulbuLator/issues/84)** —
  точная замена ULA опцией, в стороне от обычного видеотракта.
- **Ещё машины** — C64 после ATM Turbo 2.
- **Ещё звуковые форматы** — трекеры и дампы регистров, а также YM2203 из TurboSound-FM.
- **Sinclair Interface 2 и геймпады Sega / Денди.**
- **Сохранение состояния** — снимки сегодня грузятся, но записать их обратно пока нельзя.

Там, где эталоны противоречат друг другу, ответом становится переключатель в настройках машины, а не
решение, принятое за тебя; гейт SAA1099 на порту `#FF`, сервисная страница под TR-DOS и фаза контеншена портов — всё это переключатели ровно поэтому. Что Пентагон отдаёт при ЧТЕНИИ порта `#FF` — следующий на очереди: клоны расходятся, значит и это станет опцией.
...
[**Шаг 15**](research/15-pentagon/) — это момент, когда платформа стала *семейством*: одно ядро несёт **ZX Spectrum 48K, 128K и Пентагон 128–1024K**, и две синклеровские машины сверены с настоящими не на глаз, а измерением (у Пентагона приёмка таймингов ещё впереди). Наборы тестов таймингов проходят целиком на 48K (включая тесты портов) и на 128K; процессор проходит `z80full`, `z80ccf` и `z80memptr`; контеншен, момент записи в бордюр и положение прерывания совпали с железом. Подросли накопители — TR-DOS читает *и пишет*, NEMO-IDE читает `.hdf` байт в байт, работает DivMMC / esxDOS, — а General Sound играет музыку. Попутно выяснилось, что четыре дампа ПЗУ, десятилетиями ходящие по сети, испорчены — из страницы
TR-DOS осыпалось 18 байт, и один из них молча убивал запись каталога, — так что выложенные наборы
починены, и каждый байт починки описан. Готовые ядра лежат в [`research/15-pentagon/bitstreams/`](research/15-pentagon/bitstreams/), наборы ПЗУ с полным описанием происхождения — в [`research/15-pentagon/roms/`](research/15-pentagon/roms/).

Шаг 15 закрыл и четыре задачи в трекере: [#43](https://github.com/Alex-Electron/ZX_BulbuLator/issues/43) (Пентагон 128/256/1024), [#52](https://github.com/Alex-Electron/ZX_BulbuLator/issues/52) (TR-DOS / Beta Disk), [#53](https://github.com/Alex-Electron/ZX_BulbuLator/issues/53) (esxDOS через DivMMC, `.hdf` через NEMO-IDE) и зонтичную [#51](https://github.com/Alex-Electron/ZX_BulbuLator/issues/51) (виртуальные диски). Что ещё не сделано по Пентагону, собрано в [#96](https://github.com/Alex-Electron/ZX_BulbuLator/issues/96).

Дальше — **шаг 16**, новая машина ATM Turbo 2. У неё есть то, что нужно CP/M и чего нет у обычного Спектрума: ОЗУ по адресу `#0000` и текстовый режим 80×25. CP/M получает настоящий экран на 80 колонок, а Turbo Pascal запускается прямо на плате. Это будет отдельное ядро: оно грузится с карты, как ядро NES.

Потом сеть. **Шаг 17** выводит на плату веб-пульт: живой экран, клавиатура и файловый менеджер карты прямо в браузере. Сторона прошивки уже написана; препятствие физическое — Ethernet этой платы висит на выводах ПЛИС, а не процессора, поэтому нужен битстрим, выводящий контроллер на ноги микросхемы PHY.
...
- **[Шаг 15 — семейство машин Спектрума: точность по приборам](research/15-pentagon/).** Одно ядро
  несёт ZX Spectrum 48K, 128K и Пентагон 128–1024K; NES/Денди идёт отдельным ядром. Смысл этого шага в том,
...
- **Шаг 16 — ATM Turbo 2 и CP/M (в планах).** Новая машина для CP/M с текстом на 80 колонок: страничность
  ATM с ОЗУ по адресу `#0000`, текстовый режим 80×25, графика 640×200 и 320×200, турбо 7 МГц. Задача —
  [#109](https://github.com/Alex-Electron/ZX_BulbuLator/issues/109).
- **Шаг 17 — сеть (в планах).** Веб-пульт к плате: живой экран, клавиатура и файловый менеджер карты в
...
## Журнал изменений
- **25.09.2026 — Пентагон сверен с живой машиной: ядро B0200, прошивка v0.15.448.** У Пентагона наконец появился автоматический тест. [PENTTEST](research/15-pentagon/tools/penttest/) — самопроверяющаяся лента в духе Timing Tests 48K на замерном коде Яна Бобровского: длина кадра, `EI`, длина INT, вектор IM2, порт `#FF`, объём ОЗУ и отсутствие контеншена в каждом банке. Проходит 9 из 9. Две растровые ленты сами перебирают такты в stime и btime Бобровского, а скрипт на хосте читает результат по снимкам кадра. С живым Пентагоном они совпадают до полутакта: btime 17762 (у живого 17762–17763), stime 17984 виден, 17985 нет (у живой машины 17984 мерцает). Там, где эталоны расходятся, у машины теперь опции вместо одного зашитого ответа. *Pentagon INT length* по умолчанию 36 T (Fuse, ZEsarUX), можно поставить 32 T (Sizif-512, MiSTer, Unreal) или 44 T (намерено на живой плате). *Port #FF read* по умолчанию отдаёт `#FF`, как стоковый Пентагон, а может отдавать текущий атрибут, как Sizif-512 и доработанные платы, — для синклеровских игр, которые синхронизируются по плавающей шине. Лента-счётчик кадров заодно измерила частоту процессора: 3,5416 МГц на всех машинах, примерно на 1 % быстрее настоящих 48K и Пентагона.

- **24.09.2026 — прошивка v0.15.446.** *Load via (128K) = 48 LOCK* на Пентагоне теперь действительно запирает страничность. У Пентагона 1024 бит 5 `#7FFD` — замок только в стандартном режиме 128K (`#EFF7` бит 2); после сброса машина в режиме 1024, где тот же бит выбирает банк памяти. Автостарт теперь сначала включает стандартный режим, а потом запирает, как настоящая машина.

- **24.09.2026 — Шаг 15: смешение кадров, ядро B0198, прошивка v0.15.445.** Демки, которые каждый кадр переключают экран (вращающаяся тень в Eklhaft SP2, gigascreen-картинки), на ЭЛТ сливаются в ровную картинку, а на ЖК мерцают с частотой 25 Гц. *Display → Frame blend* теперь показывает среднее двух последних кадров машины на HDMI: OFF, AUTO (по умолчанию: включается только тогда, когда машина переключает экраны почти каждый кадр, чтобы игры и полосы загрузки оставались чёткими) или ON. Буферов кадра стало пять вместо трёх, так что пара всегда состоит из двух последовательных кадров машины, даже когда машина с частотой 50,02 Гц обгоняет HDMI с его 50,00 Гц на кадр. Машина и её тайминги не тронуты: без смешения вывод пиксель в пиксель прежний, а Timing Tests проходят 3/3 на 48K и 68/68 на 128K.

- **23.09.2026 — Шаг 15, прошивка v0.15.444.** При переключении машин в меню теперь применяются настройки NEMO-IDE, General Sound и мыши самой новой машины. Раньше настройки оставались такими, как их оставила предыдущая машина: 48K, 128K и Пентагон делят одно ядро, при переключении ПЛИС не перезагружается, и эти три устройства сохраняют свои собственные регистры. Тот же разбор объяснил сбои в тестах 35-37 Timing Tests 48K. На машине 48K был включён NEMO-IDE, и, как и оригинальная карта, он дешифрует порты неполно (достаточно A1 = A2 = 0), поэтому отвечает на четверть всех портов — именно на те, через которые эти тесты читают плавающую шину. Когда NEMO-IDE выключен, тест проходит три раза из трёх. Скорость ленты теперь — NORMAL или FAST 8x; режим 4x исчез, так как всё загружается со скоростью 8x. Новая опция *Load via (128K)* определяет, как автостарт входит в загрузку на 128K и Пентагоне: через меню 128, `USR 0` (48 BASIC с открытой страничностью — для 128K-демок, которые требуют `LOAD ""` из 48 BASIC) или 48 BASIC с запертой страничностью. В диалоговом окне копирования фокус перемещается на кнопку OK, как только выбран каталог в дереве, а Пентагон в меню называется просто *PENTAGON*, потому что объём его памяти — это настройка.

...
|---|---|---|
| `bitstreams/BOOT_B0196_v0.15.444.BIN` | `BOOT.BIN` | загрузочный образ: начальный загрузчик, ядро ZX и прошивка оболочки |
| `bitstreams/ATLAS_B0196.bit.bin` | `CORES/ATLAS.BIT.BIN` | ядро ZX; оболочка перезагружает его отсюда при смене машины |
...
**Лента.** `.tap`, `.tzx`, `.wav` и `.mp3`. Стандартные блоки грузятся мгновенно: SMART LOAD подаёт байты прямо в загрузчик ПЗУ, а не проигрывает ленту. Всё остальное идёт настоящими импульсами, в родном темпе или с разгоном 8×. На 128K и Пентагоне автостарт входит в загрузку через меню 128, `USR 0` или 48 BASIC с запертой страничностью — как нужно программе.

**Видео.** HDMI 720p50 с целочисленным увеличением на каждую машину, кроп по сторонам и панорама.

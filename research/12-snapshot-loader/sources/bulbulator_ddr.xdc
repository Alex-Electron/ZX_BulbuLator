...
#   keyboard scancode async_fifo (kbd_fifo_i) gray-pointer synchronisers + RAM data crossing
...
set_false_path -through [get_pins  -hierarchical -filter {NAME =~ *kbd_fifo_i*mem*/O*}]
...
set_false_path -to [get_cells -hierarchical -filter {NAME =~ *ddrdisp*nib_q_reg*}]
set_false_path -to [get_cells -hierarchical -filter {NAME =~ *ddrdisp*in_pic_q_reg*}]
set_false_path -to [get_cells -hierarchical -filter {NAME =~ *ddrdisp*have_q_reg*}]

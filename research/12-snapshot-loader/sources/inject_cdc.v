...
(
    ...
    input  wire        ctl_reset,           // machine RESET+wipe pulse (aclk)
    ...
    output reg         halt_ack,
    output reg         ram_busy,
    output reg         reset_busy,          // RESET/wipe in progress (aclk; synced from spclk)
    ...
    output reg         arm_reset_sp,        // RESET+wipe pulse to the core reset FSM (spclk)
    input  wire        reset_busy_sp        // clr_active | rst countdown (spclk, from top)
);
    //---------------------------------------------------------------------------------------------
    ...
    reg        dcom_tog = 1'b0, pcom_tog = 1'b0, rcom_tog = 1'b0;
    reg [211:0] dir_lat = 212'd0;
    reg [5:0]  p7_lat   = 6'd0;
    reg [2:0]  bd_lat   = 3'd0;
    always @(posedge aclk) begin
        if (!aresetn) begin req_tog <= 1'b0; dcom_tog <= 1'b0; pcom_tog <= 1'b0; rcom_tog <= 1'b0; end
        else begin
            ...
            if (ctl_reset)       rcom_tog <= ~rcom_tog;     // RESET+wipe request -> spclk
        end
    end
    ...
    (* ASYNC_REG="TRUE" *) reg [2:0] rsync = 3'd0;
    reg       ack_tog  = 1'b0;
    reg [3:0] hcnt     = 4'd0;
    ...
    always @(posedge spclk) begin
        ...
        if (rsync[2] ^ rsync[1]) arm_reset_sp <= 1'b1;   // RESET+wipe pulse - ungated (machine reset any time)
    ...
    (* ASYNC_REG="TRUE" *) reg [1:0] rb_sync   = 2'd0;
    always @(posedge aclk) begin
        ack_sync  <= {ack_sync[1:0], ack_tog};
        hack_sync <= {hack_sync[0], halt_ack_sp};
        halt_ack  <= hack_sync[1];
        rb_sync    <= {rb_sync[0], reset_busy_sp};
        reset_busy <= rb_sync[1];

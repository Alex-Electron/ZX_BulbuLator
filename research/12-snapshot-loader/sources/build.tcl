# Bulbulator Step 12 - Step 11 SD-browser design + the AXI-RESET path (snapshot loader), xc7z010clg400-1.
#
...
#
# vs Step 11 the delta is the AXI-RESET path: axi_ctl.v gains CONTROL bit2 (RESET+wipe) / STATUS bit2
# (reset_busy), inject_cdc.v crosses the reset request + busy back, and bulbulator_zx_ddr_top.v ORs the
# request into the existing F11 cold-reset/wipe FSM so the ARM resets the machine before each snapshot
# inject. VERSION 0xB01B0009. The per-line display + OSD compositor + DDR chain are unchanged (Step 11/8).

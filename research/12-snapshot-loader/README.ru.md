...
`.z80` (v1/v2/v3, 48K и 128K) и `.sna` (48K и 128K). Обнаружение и разбор на уровне байтов настолько сложны, что им посвящён отдельный документ: **[`docs/LOADER_SPEC.ru.md`](../../docs/LOADER_SPEC.ru.md)** — это исчерпывающее руководство: там описаны все поля заголовков обоих форматов, схема RLE, полная таблица аппаратных режимов, сопоставление страниц и банков, фишка со стеком в `.sna`, а также точно объяснено, что делает BulbuLator и что допускает сам формат. Вкратце:

...
# 5) AXI control-plane CDC (inject_cdc): aclk(100MHz) <-> spclk(56.7MHz). 1-bit synchronisers
#    (first capture FF) + toggle-synced multi-cycle data (addr_lat/data_lat -> arm_*).
set_false_path -to [get_cells -hierarchical -filter {NAME =~ *inj_i*halt_sync*}]
set_false_path -to [get_cells -hierarchical -filter {NAME =~ *inj_i*req_sync*}]
set_false_path -to [get_cells -hierarchical -filter {NAME =~ *inj_i*ack_sync*}]
set_false_path -to [get_cells -hierarchical -filter {NAME =~ *inj_i*hack_sync*}]
set_false_path -from [get_cells -hierarchical -filter {NAME =~ *inj_i*addr_lat_reg*}] -to [get_cells -hierarchical -filter {NAME =~ *inj_i*arm_*}]
set_false_path -from [get_cells -hierarchical -filter {NAME =~ *inj_i*data_lat_reg*}] -to [get_cells -hierarchical -filter {NAME =~ *inj_i*arm_*}]

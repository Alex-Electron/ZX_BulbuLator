...
- **Этап 1 — установка связи по AXI.** Крошечный автономный битовый поток (`m1-handshake-test/`,
...
- **Этап 2 — остановка + запись в память.** В полном битовом потоке ARM устанавливает `CONTROL.HALT`,
...
- **`bootgen` для образа SD очень привередлив к glibc.** На самом новом хосте (glibc 2.43) полная
  ...
  только с битовым потоком (`-process_bitstream bin`, используется для пути PCAP) не затронут. Пока что
  `BOOT.BIN` собирается там, где установлена старая версия glibc; надежное решение — открытый
  `mkbootimage`, у которого нет такой зависимости.
...
Скрипт `m1-handshake-test/axi_flash_test.sh` как раз и запускает этот процесс (Vivado Lab управляет целевым устройством XVC, а `xsdb` записывает небольшой битовый поток и выполняет команды `mrd`/`mwr`). Если всё прошло успешно, путь AXI PS↔PL исправен, и ты можешь смело приступать к интеграции.
...
Автономный битовый поток Milestone-1 собирается из файла `m1-handshake-test/build_axi_test.tcl` (не требуется ни ПЗУ, ни ядро Atlas). Если хочешь просто запустить программу, в комплекте уже есть готовые файлы `bulbulator_zx_z010.bit` и `flash/BOOT.BIN`.

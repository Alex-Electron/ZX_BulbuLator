...
//   0x64 cap_geom   R   frame-geometry probe: captured line count + first/last visible line
//                      (used to tune the display crop against a reference, e.g. ZEsarUX).
//   0x68 OSD_BG     RW  OSD panel background colour, 24-bit RGB (the alpha blend uses it).
//   0x6C OSD_OP     RW  OSD panel opacity alpha 0..255 (255 = solid, 0 = clear).
//   0x70 OSD_POS    RW  OSD panel position: [10:0] = X0 (left), [26:16] = Y0 (top).
//
...
//-------------------------------------------------------------------------------------------------
module axi_ctl #(
    parameter [31:0] VERSION    = 32'hB01B0008,

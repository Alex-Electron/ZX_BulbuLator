#!/bin/bash
# build_boot.sh  -  build the SD-card BOOT.BIN (FSBL + browser bitstream + the browser ARM app), VM-free.
#
...
#
# Inputs (all local to flash/): fsbl.bin (reused unchanged since Step 6/7), ../bulbulator_zx_browser.bit
# (this step's bitstream), and browser.bin (the browser ARM app, loaded at 0x00100000). browser.bin is
# shipped; to regenerate it from the prebuilt ELF:
#     arm-none-eabi-objcopy -O binary ../arm/browser.elf browser.bin
set -e
DIR=$(cd "$(dirname "$0")" && pwd)
BG=${BOOTGEN:-/tools/Xilinx/Vivado/2023.1/bin/bootgen}
cd "$DIR"

[ -f browser.bin ] || arm-none-eabi-objcopy -O binary ../arm/browser.elf browser.bin

"$BG" -arch zynq -image bulb_browser_sd_bin.bif -w -o BOOT_raw.bin
...
PY
rm -f BOOT_raw.bin
echo ">>> BOOT.BIN (with the browser app) ready -> copy to the SD card's FAT 'boot' partition."

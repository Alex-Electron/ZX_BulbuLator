#!/bin/bash
# browser_run.sh - flash the Step 11 bitstream over JTAG (PCAP "armoured train"), then load + run the
# browser ARM app (arm/browser.elf) on Cortex-A9 #0. The Spectrum comes up on HDMI and the OSD is live:
...
# shipped prebuilt (arm/browser.elf); it is NOT rebuilt here (see the README's honest note - a clean
...
[ -f "$HERE/arm/browser.elf" ] || { echo "arm/browser.elf missing - see arm/build_browser.sh"; exit 1; }

# 1. bootgen the PCAP .bit.bin (dense bitstreams fail over plain JTAG config on this board)
echo ">>> bootgen .bit.bin ..."
( cd "$HERE/flash" && "$BG" -arch zynq -image bulb_browser_pcap.bif -w -process_bitstream bin ) >/tmp/bg.log 2>&1 \
  && echo "    OK $(ls -la $HERE/bulbulator_zx_browser.bit.bin|awk '{print $5}') bytes" \
...
echo ">>> PCAP config ..."
export PCAP_BIN="$HERE/bulbulator_zx_browser.bit.bin"
"$XSDB" "$HERE/flash/pcap_load.tcl" 2>&1 | grep -E "PS7_INIT|PCFG_DONE|POST_CONFIG|FAIL|DDR"

# 4. load + run the browser ARM app, read back VERSION (expect 0xB01B0008)
echo ">>> load arm/browser.elf ..."
cat > /tmp/runbrowser.tcl <<TCL
connect -url tcp:localhost:3121
targets -set -filter {name =~ "*Cortex-A9*#0"}
configparams force-mem-accesses 1
catch {stop}
puts "VERSION [format 0x%08X [lindex [mrd -value 0x40000000] 0]]"
dow $HERE/arm/browser.elf
con
after 1500
puts "MACHINE_ID [format 0x%08X [lindex [mrd -value 0x40000060] 0]]"
TCL
"$XSDB" /tmp/runbrowser.tcl 2>&1 | grep -E "VERSION|MACHINE_ID|no target"
pkill -9 -x vivado_lab 2>/dev/null
echo ">>> BROWSER-RUN END - press F5 (browser) / F9 (options) / F1 (help) on the PS/2 keyboard"

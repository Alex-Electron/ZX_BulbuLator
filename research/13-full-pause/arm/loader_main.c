...
static uint8_t snapbuf[160*1024];   /* whole snapshot file (128K .sna ~131 KB) */
...
static void browser_enter(void){
    ...
    bcursor=0; btop=0;                    /* changed directory -> cursor to top (re-open keeps position) */
...
static void pause_toggle(void){
    if(paused){
        paused = 0;
        OSD_CTRL = 0;             /* hide the PAUSE badge */
        IJ_CTRL  = 0;             /* deassert HALT -> machine + audio resume exactly where they stopped */
        return;
    }
    if(osd_on) return;           /* a menu is open: nothing is running in focus -> ignore Pause */
    IJ_CTRL = 1;                 /* assert HALT: freeze the Z80 (AY/beeper clk-en gate off -> RTL mutes) */
    while(!(IJ_STAT & 1u)){}     /* wait HALT_ACK */
    paused = 1;
    osd_clear();
    draw_text_c(36, 3, "PAUSE");
    draw_text_c(96, 1, "PRESS PAUSE TO RESUME");
    osd_blit();
    OSD_CTRL = 1;                /* show the badge on the (was-closed) OSD panel */
}
...
void main(void){
    ...
    for(;;){
        ...
        if(paused) continue;                       /* modal: while paused, only Pause / F10 act */

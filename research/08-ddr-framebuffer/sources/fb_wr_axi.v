...
// DDR as 16-beat INCR bursts at `base`. WORDS words = one frame (WORDS = a multiple of 16, so every
// frame is exactly BURSTS full bursts). On the WORDS-th word it pulses frame_done and re-latches
// `base` (the buffer manager advances the write buffer on frame_done) for the next frame.
// FWFT FIFO: fifo_dout is valid while !fifo_empty; fifo_rd pops on the W handshake.
//-------------------------------------------------------------------------------------------------
module fb_wr_axi #(
    parameter [12:0] WORDS  = 13'd6480,
    parameter [8:0]  BURSTS = 9'd405
)(
    input  wire        clk,
    input  wire        resetn,
    ...
    assign aw_id=6'd0; assign aw_len=4'd15; assign aw_size=3'b011; assign aw_burst=2'b01;
    ...
    assign w_last  = (state==S_W) && (beat==4'd15);
            ...
            case (state)
                ...
                S_W: begin
                    if (w_valid && w_ready) begin            // one W beat accepted
                        ...
                        if (beat==4'd15) state<=S_B;
                        else beat <= beat + 4'd1;
                    end
                end
                S_B: if (b_valid) begin
                    if (burst==BURSTS-1) begin               // frame complete

...
Команда `./build.sh nosnow` синтезирует с параметром `-verilog_define NO_SNOW`, а скрипт `ddr_inject_nosnow_run.sh` вставляет файл `.z80` в битстрим без режима «snow» через JTAG. (Как только клавиатура подключена, это превращается в переключение клавиши во время работы, так что один битстрим выполняет обе функции.)

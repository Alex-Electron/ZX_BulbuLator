...
# PCAP image: generate <bit>.bin inline from the committed bitstream (never a stored file).
...
if [ "$(ss -ltn 2>/dev/null | grep -c :2542)" = "0" ]; then
  sudo -n pkill -9 -x "$(basename "$XVCD")" 2>/dev/null; sleep 1; sudo -n rm -f /tmp/xvcd.log

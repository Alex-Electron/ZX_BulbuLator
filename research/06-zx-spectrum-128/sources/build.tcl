# Bulbulator Step 6 - ZX Spectrum 128K + HDMI, xc7z010clg400-1.
#
# Run from the assembled build/ dir (assemble.sh populates it; see ../README.md):
#   cd build && vivado -mode batch -source build.tcl
#
# All paths are relative to build/: the Atlas core is the symlink zx/ and the HDMI
# core is hdmi/ (both -> ../../../../cores, fetched by get_deps.sh); rom128.hex, the
# board glue and the constraints are flat in build/. $readmemh reads rom128.hex from
# the working dir, which is why the build runs from inside build/.
...
read_verilog [list clock_zx.v mem_zx.v framebuffer.v kbd_buttons.v bulbulator_zx_top.v]

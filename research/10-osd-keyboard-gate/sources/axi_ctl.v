...
//-------------------------------------------------------------------------------------------------
module axi_ctl #(
    parameter [31:0] VERSION    = 32'hB01B0006,
    ...
    output reg  [8:0]  ctl_osd_waddr,     // OSD buffer word address (512 words)
    ...
    reg [8:0]  osd_ptr;                 // running OSD-buffer word pointer (auto-inc)
    ...
    always @(posedge aclk) begin
        ...
        if (!aresetn) begin
            ...
            ctl_osd_enable <= 1'b0; osd_ptr <= 9'd0; ctl_osd_waddr <= 9'd0; ctl_osd_wdata <= 32'd0;
            kbd_deadman_kick <= 1'b0;
        end else begin
            case (wstate)
                W_IDLE: begin
                ...
                W_DATA: if (s_wvalid && s_wready) begin
                    case (awidx_q)
                        ...
                        IDX_OSDADDR: osd_ptr        <= s_wdata[8:0];
                        IDX_OSDDATA: begin
                            ctl_osd_wdata <= s_wdata;
                            ctl_osd_we    <= 1'b1;
                            ctl_osd_waddr <= osd_ptr;
                            osd_ptr       <= osd_ptr + 9'd1;
                        end
        ...
        if (!aresetn) begin
            ...
            R_DATA: begin
                ...
                case (aridx_q)
                    ...
                    IDX_OSDADDR: s_rdata <= {23'd0, osd_ptr};

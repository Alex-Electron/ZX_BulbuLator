...
import os
HOST=os.environ.get("SNAPHOST","thinkpad")   # локальный запуск: SNAPHOST=localhost
XSDB=os.environ.get("XSDB","/tools/Xilinx/Vivado_Lab/2023.1/bin/xsdb")
...
set nb [rd 0x0F700014]
if {$nb < 3 || $nb > 5} { set nb 3 }
for {set k 0} {$k < $nb} {incr k} { mrd -force -bin -file /tmp/snap_b$k.bin [expr {0x0F900000 + $k*0x10000}] %d }
puts "NB=$nb"
puts SNAP_OK
""" % (FSZ//4)
def rows(raw):
...
def main():
    ...
    out=subprocess.run(["ssh",HOST,"timeout 600 %s /tmp/_s14.tcl 2>&1 | tail -3"%XSDB],capture_output=True,text=True).stdout
    if "SNAP_OK" not in out: sys.exit("снимок не снялся: "+out)
    print(out.strip().split("\n")[0])
    import re as _re
    nb=int(_re.search(r"NB=(\d+)",out).group(1)) if "NB=" in out else 3   # B0198: у ядер с 5 буферами кадра копий пять
    frames=[]
    for k in range(nb):
    ...
    pick=None
    for a in range(len(h)):             # любая копия, совпавшая с другой, - целый кадр
        if any(h[a]==h[b] for b in range(len(h)) if b!=a): pick=a; break
    if pick is None:

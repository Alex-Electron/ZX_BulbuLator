...
(
    ...
    wire warp_active;                                       // fast-load: whole-core 4x warp (defined below from tape state)
    clock_zx clock_zx_i (
        .fclk100(fclk100), .warp(warp_active), .clock(spclk), .power(sp_lock),
    ...
    wire [7:0]   ctl_vol;
    wire         ctl_pentagon, ctl_model48;     // 0xBC MACHINE_CFG: bit0 Pentagon timing, bit1 48K model (aclk)
    ...
    axi_ctl #(.VERSION(32'hB01B0040)) ctl (
        ...
        .ctl_pentagon(ctl_pentagon), .ctl_model48(ctl_model48), .ctl_pent_int(ctl_pent_int),
    ...
    always @(posedge spclk) begin fm_s0 <= ctl_tape_fmode; fm_s1 <= fm_s0; end
    ...
    wire warp_fmode     = (fm_s1 == 2'd1) | (fm_s1 == 2'd2);                // FAST(1) or SAFE(2) selected
    ...
    always @(posedge spclk) begin
        if (!warp_guard) begin                                             // tape stopped/finished OR non-warp mode -> release
            warp_latch <= 1'b0;
            ...
            if      (load_confirmed) warp_latch <= 1'b1;                   // confirmed loading -> hold warp ON
    ...
    assign warp_active = (fm_s1 == 2'd2) & warp_latch;   // SAFE -> clock_zx whole-core 4x
    wire cpuwarp_req   = (fm_s1 == 2'd1) & warp_latch;   // FAST -> CPU-only 8x mux below
    ...
    always @(posedge spclk) tsync_s <= {tsync_s[0], ctl_tape_sync};
    ...
    wire       sync_guard = trun_s[1] & tsync_s[1];            // tape running AND SYNC selected
    ...
    wire tape_advance = pe3M5_core & (~tsync_s[1] | ~tape_sync_rom | sync_hold | (sync_state == SYNC_CUSTOM));
    ...
    reg cpuw_active = 1'b0, cpuw_tog = 1'b0;
    ...
    wire pe3M5_core = pe_sel & ~cpu_halt_sp & ~rt_halt & ~cpu_starve & ~clr_active;  // gate for HALT/wipe/ROM-trap/underrun-backpressure
    ...
    (* ASYNC_REG="TRUE" *) reg [1:0] pentagon_s = 2'b00, model48_s = 2'b00;
    always @(posedge spclk) begin
    ...
    wire tape_sample;                                      // core: 1 while the CPU reads port 0xFE (loader sampling the ear)
    wire cpu_ten_sp;                                       // core: contended CPU T-state enable (pc3M5) - real CPU advance rate (128K: stalls on contention; Pentagon: == pe3M5)

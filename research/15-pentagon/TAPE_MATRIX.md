...
Трекер надёжности загрузки лент. Ведёт AI (учёт), исполняет the second agent + AI (hardware-прогоны).
...
Текущий борд — **0x40 (BRAM PL4K)**. Их надо **ПЕРЕПРОВЕРИТЬ на 0x40** (сам the second agent отметил как remaining).
...
## MUST-PASS СПЕКА (дословно из вердикта the second agent 00:05 — это blocker, не nice-to-have)
...
Поля трекера (по the second agent): file / format / speed / sync / policy / count / hash / gaps / resumes / PC / screen / repetitions.
...
- ✅→⏳ PCM s16/48 kHz и CBR/VBR MP3 fixtures СОЗДАНЫ (AI 21.07, детерминированно, command+SHA в
  `fixtures/MANIFEST.md`; стейдж `thinkpad:/tmp/fixtures/{wav,mp3}`). Осталось: записать на SD и прогнать в matrix.
  Открыто: пара MP3 «без internal pilot» — ждёт семантики preflight (the second agent вернётся 27.07, либо выведу из кода сам).
- ⛔ Провенанс: B0043 AUTO-ROM-слой отсутствует в воспроизводимых исходниках (вывод the second agent 21.07); B0049 collected-артефакт

...
module fb_wr_axi #(
    ...
    input  wire        quiesce_i,   // v158 QUIESCE: stop issuing NEW AW bursts (in-flight ones complete) - safe PL reload
    output wire        idle_o       // 1 = no write transaction in flight (safe to isolate PS<->PL)
);
    ...
    assign busy_o  = 1'b1;
    assign idle_o  = ((state==S_AW) && !aw_valid) || (state==S_WAIT);   // v158: nothing in flight
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            ...
            case (state)
                S_AW: begin
                    if (!quiesce_i) aw_valid <= 1'b1;   /* v158: no NEW bursts under quiesce (a raised VALID stays until READY per AXI) */

...
module fb_line_disp #(
    ...
    //=============================================================================================
    wire        in_v   = (cy >= vm_q) && (cy < vm_q + (ch_q<<YSH));
    wire [8:0]  row_in = sy_q[8:0] + ((cy - vm_q) >> YSH);
    reg  [8:0]  need_row;
    ...
    //=============================================================================================
    wire in_pic = (cx >= hm_q) && (cx < hm_q + (cw_q<<XSH)) &&
                  (cy >= vm_q) && (cy < vm_q + (ch_q<<YSH));
    wire [8:0]  rd_sx = sx_q[8:0] + ((cx - hm_q) >> XSH);
    ...
    wire [8:0]  rd_sy = sy_q[8:0] + ((cy - vm_q) >> YSH);

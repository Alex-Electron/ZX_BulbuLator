# Deploy pack: BOOT.BIN v0.15.152 + NES core CE09 + game carts -> SD via the loader fs-service.
...
proc run_cmd {cmd} {
    ...
    if {$done != 1 || $err != 0} { puts "CMD $cmd FAIL (done: $done, err: $err)"; return 0 }
    return 1
}
proc write_file {path local_file} {
    ...
    puts "Writing $path ($size bytes)..."
    dow -data $local_file $fs_buf
    if {[run_cmd 2]} { puts "WRITE $path SUCCESS" } else { puts "WRITE $path FAIL" }
}
# 1. mkdir 0:/NES (существующая = OK)
write_str $fs_path "0:/NES"
if {[run_cmd 5]} { puts "MKDIR 0:/NES OK" }
# 2. свежий загрузочный образ v0.15.152
...
# 4. игры (8.3-имена)
write_file "0:/NES/TANK1990.NES" "/home/lavrinovich/nes_games/tank1990.nes"
write_file "0:/NES/SMB.NES" "/home/lavrinovich/nes_games/smb.nes"
puts ">>> NES PACK DEPLOYED"
exit

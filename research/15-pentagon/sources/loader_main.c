...
static int   opt_scr_x       = 256;      /* live: whole-frame H position on HDMI (fb_line_disp HMARGIN); moves border+paper together */
static int   opt_scr_y       = 58;       /* live: whole-frame V position on HDMI (fb_line_disp VMARGIN) */
...
static void cfg_set(const char* k, const char* v){
    ...
    else if(!cicmp(k,"paper_h")){ int d=0; for(const char*p=v;*p>='0'&&*p<='9';p++) d=d*10+(*p-'0'); if(d<0)d=0; if(d>127)d=127; opt_paper_h=d; }
    else if(!cicmp(k,"paper_v")){ int d=0; for(const char*p=v;*p>='0'&&*p<='9';p++) d=d*10+(*p-'0'); if(d<0)d=0; if(d>63)d=63; opt_paper_v=d; }
    else if(!cicmp(k,"scr_x")){ int d=0; for(const char*p=v;*p>='0'&&*p<='9';p++) d=d*10+(*p-'0'); if(d<0)d=0; if(d>640)d=640; opt_scr_x=d; }
    else if(!cicmp(k,"scr_y")){ int d=0; for(const char*p=v;*p>='0'&&*p<='9';p++) d=d*10+(*p-'0'); if(d<0)d=0; if(d>200)d=200; opt_scr_y=d; }
}
...
static int config_save(void){                  /* 1 = written OK, 0 = failed (card RO/full/removed) */
    ...
    { char b[8]; itoa_u(opt_paper_h,b); p=appstr(o,p,"paper_h="); p=appstr(o,p,b); o[p++]='\r'; o[p++]='\n'; }
    { char b[8]; itoa_u(opt_paper_v,b); p=appstr(o,p,"paper_v="); p=appstr(o,p,b); o[p++]='\r'; o[p++]='\n'; }
...
static menu_item opt_items[] = {
    ...
    {"DEF MACHINE", ITEM_CHOICE, &opt_defmachine, CH_MACHINE, N_MACHINES, 0, apply_machine},   /* [20] Step 15: default machine - changing this will cold-reboot (use only when ready) */
...
static const MenuItem mi_settings[] = {
  ...
  {"Paper H off",    0,0,NULL, NULL, &opt_items[23]},   /* Step 15: paper within frame - H (border distribution) */
  {"Paper V off",    0,0,NULL, NULL, &opt_items[24]},   /* Step 15: paper within frame - V */
  {"Screen X",       0,0,NULL, NULL, &opt_items[25]},   /* Step 15: whole frame (border+paper) H position on HDMI */
  {"Screen Y",       0,0,NULL, NULL, &opt_items[26]},   /* Step 15: whole frame V position on HDMI */
  {"Pentagon INT V", 0,0,NULL, NULL, &opt_items[21]},   /* Step 15: TIMING (not position!) - reference 239; changing it re-times border FX -> reload the demo after */
  {"Pentagon INT H", 0,0,NULL, NULL, &opt_items[22]},   /* Step 15: TIMING - reference 326; same caveat */
...
static Menu m_settings = { mi_settings, 17, 0 };
...
void main(void){
    ...
    apply_paper();                    /* Step 15: paper position within frame (from ini) */
    apply_scr();                      /* Step 15: whole-frame HDMI position (from ini) */

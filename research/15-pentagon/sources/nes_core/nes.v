...
module NES(
	...
	output        ppumem_vram_ce,   // BulbuLator: CIRAM chip-enable (nametable vs cart CHR) for external 2KB nametable RAM
	output        ppumem_vram_a10,  // BulbuLator: CIRAM A10 (nametable mirroring)
...
wire is_cpu_ram = (addr <= 16'h1FFF);
assign cpumem_addr  = is_cpu_ram ? {1'b1, 10'b0, addr[10:0]} : {1'b0, prg_linaddr[20:0]};
assign cpumem_read  = is_cpu_ram ? mr_int : ((prg_read & prg_allow) | (prg_write && prg_conflict));
assign cpumem_write = is_cpu_ram ? mw_int : (prg_write && prg_allow);
assign cpumem_dout  = prg_din;
assign ppumem_addr  = chr_linaddr;
assign ppumem_read  = chr_read;
assign ppumem_write = chr_write && (chr_allow || vram_ce);
assign ppumem_vram_ce  = vram_ce;   // BulbuLator: route CIRAM select/mirroring out to nes_mem_bram (2KB nametable RAM)
assign ppumem_vram_a10 = vram_a10;
...
always @* begin
	...
	end else if (is_cpu_ram || prg_allow) begin

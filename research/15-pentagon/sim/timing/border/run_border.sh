#!/bin/bash
# run_border.sh - собрать (при необходимости) и запустить стенд tb_border (копия харнеса + монитор).
#   ./run_border.sh [--build] [--wd=<имя>] NAME=VAL ...
# Список файлов = harness/run.sh (build.tcl цели Atlas). gen/wd1793_xsim.sv берётся из харнеса.
set -e
...
[ -d xsim.dir/tb_border ] || BUILD=1

if [ $BUILD = 1 ]; then
  echo "== компиляция tb_border"
  python3 make_tb.py
  [ -f "$HARN/gen/wd1793_xsim.sv" ] || { echo "нет $HARN/gen/wd1793_xsim.sv - сначала harness/run.sh --build"; exit 1; }
  ...
  xvlog -sv tb_border.sv > logs/xvlog_tb.log
  xelab tb_border -s tb_border --timescale 1ps/1fs > logs/xelab.log
  grep -q "ERROR" logs/xelab.log && { echo "ошибки в logs/xelab.log"; grep ERROR logs/xelab.log; exit 1; } || true
...
T0=$(date +%s.%N)
xsim tb_border -R "${ARGS[@]}" > "$LOG" 2>&1
T1=$(date +%s.%N)
printf "wall: %.1f s\n" "$(echo "$T1 - $T0" | bc)" >> "$LOG"
echo "готово: $LOG ($(tail -1 "$LOG"))"

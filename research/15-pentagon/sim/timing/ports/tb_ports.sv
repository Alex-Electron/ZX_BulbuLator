...
reg        dirset   = 1'b0;
reg [211:0] dir_word = 212'd0;                 // DIRFILE: слово регистров T80 (раскладка inject_finish)
reg [1023:0] dirfile = 0; integer dirfd, dirk; reg [31:0] dirw [0:6]; reg use_dirfile = 1'b0;
...
main dut (
	...
	.dirset(dirset), .dir(dir_word), .reg_out(reg_out),
...
// ФРОНТ /INT против момента выборки процессора (гипотеза Потапова о гонке на точке выборки):
// печатаем состояние разрешений в мастер-такте, где падает сырой irq, и в соседних.
integer edge_left = 0;
initial if ($value$plusargs("EDGE=%d", edge_left)) ;
reg irq_q = 1'b1; reg [4:0] edge_win = 5'd0;
always @(posedge clock) if (reset_n) begin
	irq_q <= dut.Video.irq;
	if (irq_q && !dut.Video.irq && edge_left > 0) begin edge_win = 5'd20; edge_left = edge_left - 1; $display("[ФРОНТ] irq упал: mclk=%0d hUla=%0d", my_mclk, dut.Video.hUla); end
	if (edge_win != 0) begin
		$display("[ФРОНТ]   mclk=%0d pe7M0=%b ne7M0=%b pe3M5=%b ne3M5=%b pc3M5=%b cpuck=%b vduI=%b vduI_m=%b cpu_irq=%b",
			my_mclk, dut.pe7M0, dut.ne7M0, pe3M5, dut.ne3M5, dut.pc3M5, dut.cpuck, dut.vduI, dut.vduI_m, dut.cpu_irq);
		edge_win = edge_win - 1;
	end
end

// M1T: трасса выборок команд с АБСОЛЮТНЫМ тактом процессора (my_pe) - для потактовой сверки с трассой
// оракула SkoolKit (trace.py -I TraceLine="{t} {pc:04X} {i}"). Печатаем первые M1T выборок.
integer m1t_left = 0;
initial if ($value$plusargs("M1T=%d", m1t_left)) ;
reg m1cyc_t = 1'b0;
always @(posedge clock) if (reset_n && m1t_left > 0) begin
	m1cyc_t <= m1cyc;
	if (m1cyc & ~m1cyc_t) begin $display("[M1T] %0d %04h", my_pe, dut.a); m1t_left = m1t_left - 1; end
end
...
initial begin
	...
	if ($value$plusargs("DIRFILE=%s", dirfile)) begin
		dirfd = $fopen(dirfile, "r");
		if (dirfd == 0) begin $display("DIRFILE: не открыть %0s", dirfile); $finish; end
		for (dirk = 0; dirk < 7; dirk = dirk + 1) if ($fscanf(dirfd, "%h", dirw[dirk]) != 1) begin $display("DIRFILE: мало слов"); $finish; end
		$fclose(dirfd);
		// раскладка inject_finish: [0]=A|F<<8|A'<<16|F'<<24 [1]=I|R<<8|SP<<16 [2]=PC|BC<<16 [3]=DE|HL<<16
		//                          [4]=IX|BC'<<16 [5]=DE'|HL'<<16 [6]=IY|IM<<16|IFF1<<18|IFF2<<19
		dir_word = {dirw[6][19:0], dirw[5], dirw[4], dirw[3], dirw[2], dirw[1], dirw[0]};
		use_dirfile = 1'b1;
		wait_us(1.0);
		dirset = 1'b1;
		wait_us(1.0);
		dirset = 1'b0;
		dir_word = 212'd0;
		$display("DIRFILE: регистры загружены, PC=%04h SP=%04h IM=%0d IFF1=%b", dirw[2][15:0], dirw[1][31:16], dirw[6][17:16], dirw[6][18]);
	end

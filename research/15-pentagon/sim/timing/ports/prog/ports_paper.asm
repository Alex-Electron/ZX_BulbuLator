; ports_paper.asm - тот же ports.asm, но серия IN попадает В БУМАГУ (строки ~100..104): ожидание 1500
; итераций = 36 000 T, плюс два дополнительных чтения AY в конце (800D, 800E).
; Сначала ~36 000 T ожидания (растр в бумаге, строка ~100), затем серия IN с записью
...
;   800D: IN (#FFFD) после OUT #FFFD,#07 и OUT #FFFD,#17 (адрес с битом 4)     (настоящий AY: #3F)
;   800E: IN (#FFFD) регистр 15 (порт B)
        org 9000h
start:  di
        ld sp,8ff0h
        xor a
...
w0:     dec bc
        ld a,b
        or c
        jp nz,w0             ; 24 T * 1500 = 36 000 T -> строка ~97 (48K) / ~95 (128K)
        ld a,0ffh
        in a,(0feh)
        ld (8000h),a
        ld a,10h
        ...
        ld bc,0fffdh
        ld a,7
        out (c),a            ; регистр 7 (в нём #3F)
        ld a,17h
        out (c),a            ; адрес с ненулевым старшим полубайтом: настоящий AY его НЕ защёлкивает -> остаётся R7
        in a,(c)
        ld (800dh),a         ; ожидание для настоящего AY: #3F
        ld a,0fh
        out (c),a
        in a,(c)
        ld (800eh),a         ; регистр 15 (порт B, вход не подключён)
        ld a,1

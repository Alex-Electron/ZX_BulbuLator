...
module tb_intack;
  ...
  task build(input im2, input halted);
    begin
      ...
      for (k=16'h8100;k<=16'h8201;k=k+1) mem[k]=8'h83;          // вектор IM2 -> 8181
      mem[16'h0038]=8'hD3; mem[16'h0039]=8'hFE;                 // IM1: OUT (FE),A
      mem[16'h8383]=8'hD3; mem[16'h8384]=8'hFE;                 // IM2: OUT (FE),A
    end
  endtask
  integer res;
  ...
  always @(posedge clk) begin
    m1d <= m1;
    if (0) begin nm1=nm1+1; $display("    M1 #%0d a=%04h d=%02h irq_n=%b iff?", nm1, a, d, irq_n); end
    if (0) $display("    IO WRITE a=%04h q=%02h at %0t", a, q, $time);
  end
